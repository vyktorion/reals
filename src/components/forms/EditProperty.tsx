...
export function EditProperty({ property, onClose, onPropertyUpdated }: EditPropertyProps) {
  ...
  const [formData, setFormData] = useState<PropertyFormData>({
    ...
    parkingSpaces: property.parking?.toString() || '',
  ...
  const propertyTypes = ['sale', 'rent', 'hotel'];
  const propertyStatuses = ['active', 'sold', 'rented', 'pending'];
  ...
  const handleInputChange = (field: keyof PropertyFormData, value: string | 'sqft' | 'sqm' | 'acre' | boolean) => {
  ...
  const handleSubmit = () => {
    ...
    const lotSize = formData.lotSize ? parseInt(formData.lotSize) : undefined;
    const yearBuilt = formData.yearBuilt ? parseInt(formData.yearBuilt) : undefined;
    const parkingSpaces = formData.parkingSpaces ? parseInt(formData.parkingSpaces) : undefined;

    // Validate number parsing
    if (isNaN(price) || isNaN(bedrooms) || isNaN(bathrooms) || isNaN(area)) {
      toast.error('Please enter valid numbers for price, bedrooms, bathrooms, and area');
      return;
    }
                  ...
                  <SelectContent>
                    {propertyTypes.map((type) => (
                      <SelectItem key={type} value={type}>
                        {type === 'sale' ? 'For Sale' : type === 'rent' ? 'For Rent' : 'Hotel'}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div>
                ...
                <div className="flex mt-1">
                  <Input
                    ...
                    className="flex-1"
                  />
                  <Select value={formData.areaUnit} onValueChange={(value: 'sqft' | 'sqm' | 'acre') => setFormData(prev => ({ ...prev, areaUnit: value }))}>
              ...
              <div>
                ...
                <div className="flex mt-1">
                  <Input
                    ...
                    className="flex-1"
                  />
                  <Select value={formData.lotSizeUnit} onValueChange={(value: 'sqft' | 'sqm' | 'acre') => setFormData(prev => ({ ...prev, lotSizeUnit: value }))}>
                    <SelectTrigger className="w-24 ml-2">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
