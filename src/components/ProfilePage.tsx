...
import { AccountSettings } from '@/features/user-profile/components/AccountSettings';
...
export function ProfilePage({ favoriteCount, onNavigateToSavedSearches, onNavigateToNotifications }: ProfilePageProps) {
  ...
  const handleTogglePublish = (id: string) => {
    setListings((prev) =>
      prev.map((p) => {
        if (p.id !== id) return p;
        const newStatus: Property['status'] = p.status === 'active' ? 'sold' : 'active';
        return { ...p, status: newStatus, updatedAt: new Date() };
      })
    );
                ...
                {listings.map((listing) => (
                  <li key={listing.id} className="py-5 flex items-center gap-4">
                    <div className="relative w-48 h-32 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700 shrink-0">
                      ...
                      />
                      <span className={`absolute top-2 left-2 text-[12px] px-3 py-1 rounded-md shadow-sm ${listing.status === 'active' ? 'bg-green-600 text-primary-foreground' : 'bg-background text-foreground border border-border'}`}>
                        {listing.status === 'active' ? 'Activ' : 'Inactiv'}
                      </span>
                    </div>
