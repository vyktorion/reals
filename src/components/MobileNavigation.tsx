...
export function MobileNavigation({ currentView, onViewChange, favoriteCount }: MobileNavigationProps) {
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-card border-t border-border shadow-lg safe-bottom">
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center justify-center gap-0.5 transition-all duration-200 ${
                isActive 
                  ? 'text-primary' 
                  : 'text-gray-500 dark:text-gray-400 active:bg-background dark:active:bg-gray-700'
              }`}
            >
              {/* Active Indicator */}
              {isActive && (
              ...
              <div className="relative">
                ...
                {item.badge !== undefined && item.badge > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-primary-foreground text-xs w-4 h-4 rounded-full flex items-center justify-center">
